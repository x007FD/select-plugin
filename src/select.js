export class Select {
    ...
    static SELECT_CLASS = 'ms-select'
    static EMPTY_CLASS = 'is-empty'
    static CHECKED_CLASS = 'is-checked'
    ...
    get currentLang() {
        return !this.options.lang ? Select.DEFAULT_LANG : this.options.lang.toLowerCase()
    }
    ...
    get isOpen() {
        return this.selectElement.classList.contains(Select.ACTIVE_CLASS)
    }

    clickHandler(event) {
        const target = event.target
        const targetSelectId = target.dataset.selectId

        if (target.closest('.ms-select__header')) {
            this.toggle()
        }

        if (target.closest('.ms-select__item')) {
            this.select(targetSelectId)
        }
    }
    ...
    select(id) {
        ...
        itemElements.forEach(item => {
            item.classList.remove(Select.CHECKED_CLASS)
            item.disabled = false
        })

        this.selectElement.querySelector(`.ms-select__item[data-select-id="${ this.#selectedId }"]`).classList.add(Select.CHECKED_CLASS)
    ...
    open() {
        ...
        this.selectElement.classList.add(Select.ACTIVE_CLASS)
        this.#executeEvent('onAfterOpen')
    }

    close() {
        this.#executeEvent('onBeforeClose')
        this.selectElement.classList.remove(Select.ACTIVE_CLASS)
    ...
    #renderSelect() {
        ...
        return `
          <div class="ms-select__header">
            <span class="ms-select__header-value">
              ${ placeholder }
            </span>
            <svg class="ms-select__header-icon" width="12" height="7" viewBox="0 0 12 7" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <input name="ms-select-option" type="hidden" class="ms-select__input" value="${ id }">
          </div>
    ...
    #render() {
        ...
        if (!this.data || !this.data.length) {
            this.selectElement.classList.add(Select.EMPTY_CLASS)
            console.error(translations.noItems)
        }

        this.selectElement.classList.add(Select.SELECT_CLASS)
    ...
    #getSelectElements() {
        return {
            headerValueElement: this.selectElement.querySelector('.ms-select__header-value'),
            inputValueElement: this.selectElement.querySelector('.ms-select__input'),
            itemElements: this.selectElement.querySelectorAll('.ms-select__item')
        }
    }
    ...
    #getItems() {
        ...
        return sortedItems.map((item, index) => `
              <button class="ms-select__item ${ this.#selectedId === +item.id ? Select.CHECKED_CLASS : '' }"
                      data-select-id="${ item.id }"
                      ${ this.#selectedId === +item.id ? 'disabled' : '' }>
                      <span class="ms-select__item-value">
                        ${ this.options.isOrder ? `${ index + 1 }.` : '' } ${ item.value }           
                      </span>
                      <span class="ms-select__item-info">
                        ${ item.info ?? '' }      
                      </span>
              </button>
          `).join('')
